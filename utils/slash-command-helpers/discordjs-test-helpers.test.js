...
describe('buildInteraction', () => {
  ...
  it('returns the requested user when the getUser option is called', () => {
    ...
    expect(interaction.options.getUser('user0')).toEqual({
      id: '1234',
      username: 'Foo',
      nickname: '',
    });
    expect(interaction.options.getUser('user1')).toEqual({
      id: '5678',
      username: 'Baz',
      nickname: '',
    });
...
describe('initalizeServer', () => {
  it('returns the requested user when fetch is called with their id', () => {
    ...
    expect(server.members.fetch('1234')).toEqual({
      user: { username: 'Foo' },
      nickname: 'bar',
    });
    expect(server.members.fetch('5678')).toEqual({
      user: { username: 'Baz' },
      nickname: '',
    });
  });
