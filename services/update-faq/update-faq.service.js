...
class UpdateFAQsService {
  ...
  static async handleInteraction(interaction) {
    ...
    const FAQChannel = interaction.guild.channels.cache.get(
      config.channels.FAQChannelId,
    );
  ...
  static async fetchFAQs() {
    const response = await fetch(
      'https://raw.githubusercontent.com/TheOdinProject/top-meta/main/FAQ.md',
    );
  ...
  static createFAQEmbeds(rawFAQs) {
    return UpdateFAQsService.segments(
      rawFAQs,
      UpdateFAQsService.EmbedDescriptionCharacterLimit,
      UpdateFAQsService.Delimiter,
    ).map((chunk) =>
  ...
  static segments(string, length, delimiter, prependDelimiter = true) {
    ...
    for (let i = 0; i < segmentedString.length; i += 1) {
      const segment = prependDelimiter
        ? delimiter + segmentedString[i]
        : segmentedString[i];
